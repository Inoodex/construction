...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Create Milestone — {{ $project->name }}</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.core.projects.milestones.store', $project) }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="target_date" id="target_date" class="form-input" value="{{ old('target_date') }}" />
                </div>
                <div class="form-group">
                    <label for="achieved_date">Achieved Date</label>
                    <input type="date" name="achieved_date" id="achieved_date" class="form-input" value="{{ old('achieved_date') }}" />
                </div>
            </div>
            ...
            <div class="mt-8 flex items-center gap-4">
                <button type="submit" class="btn btn-primary px-10">Save Milestone</button>
