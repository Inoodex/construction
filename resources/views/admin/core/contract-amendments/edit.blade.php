...
@section('title', 'Edit Contract Amendment')

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Edit Amendment: {{ $contractAmendment->amendment_number }}</h2>
    ...
    <form action="{{ route('admin.core.contract-amendments.update', $contractAmendment) }}" method="POST" class="mt-6">
        @csrf
        @method('PUT')
        <div class="panel grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="contract_id" class="form-select w-full" required>
                    ...
                    @foreach($contracts as $contract)
                        <option value="{{ $contract->id }}" {{ old('contract_id', $contractAmendment->contract_id) == $contract->id ? 'selected' : '' }}>{{ $contract->contract_number }} - {{ $contract->title }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="type" class="form-select w-full" required>
                    @foreach(['scope_change' => 'Scope Change', 'time_extension' => 'Time Extension', 'value_change' => 'Value Change', 'other' => 'Other'] as $val => $lbl)
                        <option value="{{ $val }}" {{ old('type', $contractAmendment->type) == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" value="{{ old('title', $contractAmendment->title) }}" class="form-input w-full" required />
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="3" class="form-input w-full" required>{{ old('description', $contractAmendment->description) }}</textarea>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="cost_impact" value="{{ old('cost_impact', $contractAmendment->cost_impact) }}" class="form-input w-full" />
            ...
            <div>
                ...
                <input type="number" name="time_impact_days" value="{{ old('time_impact_days', $contractAmendment->time_impact_days) }}" class="form-input w-full" />
            ...
            <div>
                ...
                <select name="status" class="form-select w-full" required>
                    @foreach(['draft' => 'Draft', 'submitted' => 'Submitted', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $val => $lbl)
                        <option value="{{ $val }}" {{ old('status', $contractAmendment->status) == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="requested_by" class="form-select w-full">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('requested_by', $contractAmendment->requested_by) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="3" class="form-input w-full">{{ old('notes', $contractAmendment->notes) }}</textarea>
            ...
            <div class="md:col-span-2 flex justify-end gap-2">
                ...
                <button type="submit" class="btn btn-primary">Update Amendment</button>
            </div>
        </div>
    </form>
@endsection
