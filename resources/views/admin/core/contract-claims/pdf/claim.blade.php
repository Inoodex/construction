...
<head>
    <meta charset="UTF-8">
    <title>Contract Claim - {{ $contractClaim->claim_number }}</title>
    <style>
        @page { margin: 0; }
    ...
    <div class="wrapper">
        ...
        <div class="content">
            <div style="margin-bottom:15px;">
                <div class="doc-number">{{ $contractClaim->claim_number }}</div>
            </div>

            <table class="meta-table">
                <tr>
                    <th>Title</th>
                    <td colspan="3">{{ $contractClaim->title }}</td>
                </tr>
                <tr>
                    <th>Contract</th>
                    <td>{{ $contractClaim->contract->contract_number ?? 'N/A' }}</td>
                    <th>Status</th>
                    <td style="text-transform:capitalize">{{ str_replace('_', ' ', ucfirst($contractClaim->status)) }}</td>
                </tr>
                <tr>
                    <th>Type</th>
                    <td>{{ ucfirst(str_replace('_', ' ', $contractClaim->type)) }}</td>
                    <th>Submitted Date</th>
                    <td>{{ $contractClaim->submitted_date?->format('d M Y') ?? '—' }}</td>
                </tr>
                <tr>
                    <th>Claimed Amount</th>
                    <td>{{ $contractClaim->claimed_amount !== null ? number_format($contractClaim->claimed_amount, 2) : '—' }}</td>
                    <th>Claimed Days</th>
                    <td>{{ $contractClaim->claimed_days ?? '—' }}</td>
                </tr>
                <tr>
                    <th>Granted Amount</th>
                    <td>{{ $contractClaim->granted_amount !== null ? number_format($contractClaim->granted_amount, 2) : '—' }}</td>
                    <th>Granted Days</th>
                    <td>{{ $contractClaim->granted_days ?? '—' }}</td>
                </tr>
                <tr>
                    <th>Submitted By</th>
                    <td>{{ $contractClaim->submitter->name ?? 'N/A' }}</td>
                    <th>Reviewer</th>
                    <td>{{ $contractClaim->reviewer->name ?? '—' }}</td>
                </tr>
                <tr>
                    <th>Response Date</th>
                    <td>{{ $contractClaim->response_date?->format('d M Y') ?? '—' }}</td>
                    <th>Created By</th>
                    <td>{{ $contractClaim->creator->name ?? 'N/A' }}</td>
                </tr>
            </table>

            @if($contractClaim->description)
                <div class="detail-box">
                    <h3>Description</h3>
                    <p>{{ $contractClaim->description }}</p>
                </div>
            @endif
