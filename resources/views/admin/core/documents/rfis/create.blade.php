...
@section('title', 'New RFI')

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">New Request for Information</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.core.documents.rfis.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="subject" id="subject" class="form-input" required value="{{ old('subject') }}" />
                    @error('subject') <span class="text-danger text-sm">{{ $message }}</span> @enderror
                </div>
                <div class="form-group md:col-span-2">
                    <label for="question">Question <span class="text-danger">*</span></label>
                    <textarea name="question" id="question" class="form-textarea" rows="4" required>{{ old('question') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select name="drawing_id" id="drawing_id" class="form-select">
                        ...
                        @foreach($drawings as $drawing)
                            <option value="{{ $drawing->id }}" {{ old('drawing_id') == $drawing->id ? 'selected' : '' }}>{{ $drawing->drawing_number }} — {{ $drawing->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="mt-8 flex items-center gap-4">
                <button type="submit" class="btn btn-primary px-10">Create RFI</button>
