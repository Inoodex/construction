...
@section('title', 'New Inspection Checklist')

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">New Inspection Checklist</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.core.inspection-checklists.store') }}" method="POST">
            @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="site_id" id="site_id" class="form-select" required>
                        ...
                        @foreach($sites as $site)
                            <option value="{{ $site->id }}" {{ old('site_id') == $site->id ? 'selected' : '' }}>{{ $site->name }} ({{ $site->project->name ?? '—' }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="inspection_date">Inspection Date <span class="text-danger">*</span></label>
                    <input type="date" name="inspection_date" id="inspection_date" class="form-input" required value="{{ old('inspection_date', date('Y-m-d')) }}" />
                </div>
                <div class="form-group">
    ...
    <div class="item-row grid items-center gap-3 rounded-lg border p-3 dark:border-gray-700"
        ...
        <button type="button" onclick="this.closest('.item-row').remove()"
            ...
            <div class="mt-8 flex items-center gap-4">
                <button type="submit" class="btn btn-primary px-10">Save Checklist</button>
