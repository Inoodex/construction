...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Accounts Receivable Aging</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.finance.aging.ar') }}" method="GET" class="mb-5 flex items-center gap-3 w-full">
            ...
            @if(request()->filled('project_id'))
                <a href="{{ route('admin.finance.aging.ar') }}" class="btn btn-outline-danger">Reset</a>
            @endif
        </form>
        ...
        <div class="overflow-x-auto">
            @foreach($buckets as $key => $bucket)
                @if($bucket['invoices']->count() > 0)
                    ...
                    <table class="table-hover mb-4 w-full table-auto">
                        <thead><tr><th>Invoice #</th><th>Project</th><th>Due Date</th><th>Total</th><th>Due</th><th>Days Overdue</th></tr></thead>
                        <tbody>
                            @foreach($bucket['invoices'] as $inv)
                                <tr>
                                    <td><a href="{{ route('admin.finance.invoices.show', $inv->id) }}" class="font-mono text-xs text-primary hover:underline">{{ $inv->invoice_number }}</a></td>
                                    ...
                                    <td class="font-semibold text-danger">{{ number_format($inv->due_amount) }}</td>
                                    <td class="text-xs">{{ $inv->due_date ? max(0, now()->diffInDays($inv->due_date, false)) : 0 }}d</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @endforeach
            @if($grandTotal <= 0)
                <p class="text-center py-4 text-white-dark">No outstanding receivables.</p>
            @endif
        </div>
