...
@section('title', 'Add Fuel Log Entry')

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Add Fuel Log Entry</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.hr.fuel-logs.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="equipment_id" id="equipment_id" class="form-select" required>
                        ...
                        @foreach($equipment as $eq)
                            <option value="{{ $eq->id }}" {{ old('equipment_id') == $eq->id ? 'selected' : '' }}>{{ $eq->name }} ({{ $eq->code }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="unit" class="form-select" required>
                        <option value="liters" {{ old('unit', 'liters') == 'liters' ? 'selected' : '' }}>Liters</option>
                ...
                <div>
                    ...
                    <input type="number" step="0.1" name="quantity" class="form-input" required value="{{ old('quantity') }}" min="0" />
                </div>
                <div>
                    <label class="text-sm font-semibold">Unit Cost <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" name="unit_cost" class="form-input" required value="{{ old('unit_cost') }}" min="0" />
                </div>
                <div>
                    <label class="text-sm font-semibold">Meter Hours</label>
                    <input type="number" name="meter_hours" class="form-input" value="{{ old('meter_hours') }}" min="0" />
                </div>
                <div>
                    <label class="text-sm font-semibold">Vendor</label>
                    <input type="text" name="vendor" id="vendor" class="form-input" value="{{ old('vendor') }}" />
                </div>
                <div>
                    <label class="text-sm font-semibold">Receipt No.</label>
                    <input type="text" name="receipt_no" class="form-input" value="{{ old('receipt_no') }}" />
                </div>
            </div>
            <div class="mt-4">
