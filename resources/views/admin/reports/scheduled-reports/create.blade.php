...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Schedule Report</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.reports.scheduled-reports.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <div class="form-group">
                    ...
                    <select name="report_template_id" id="report_template_id" class="form-select" required>
                        ...
                        @foreach($templates as $t)
                            <option value="{{ $t->id }}" {{ old('report_template_id') == $t->id ? 'selected' : '' }}>{{ $t->name }} ({{ $t->report_type }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" name="next_run_at" id="next_run_at" class="form-input" required value="{{ old('next_run_at') }}" />
                </div>
                <div class="form-group md:col-span-2">
                    ...
                    <div id="recipients-wrapper">
                        <div class="flex gap-2 mb-2">
                            ...
                            <button type="button" onclick="addRecipient()" class="btn btn-sm btn-outline-primary">+</button>
                        </div>
                    </div>
                    @error('recipients') <p class="text-danger text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="mt-8 flex items-center gap-4">
                <button type="submit" class="btn btn-primary px-10">Create Schedule</button>
