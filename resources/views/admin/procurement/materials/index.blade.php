...
@section('content')
    ...
    <div class="panel mt-6">
        <div class="mb-5">
            <form action="{{ route('admin.procurement.materials.index') }}" method="GET" class="flex items-center gap-3 w-full">
                <div class="relative flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name or SKU..." class="form-input ltr:pr-11 rtl:pl-11 w-full" />
                ...
                @if(request()->anyFilled(['search']))
                    <a href="{{ route('admin.procurement.materials.index') }}" class="btn btn-outline-danger">Reset</a>
                @endif
            </form>
        </div>
