...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Edit Material</h2>
    ...
    <div class="panel mt-6">
        <form action="{{ route('admin.procurement.materials.update', $material->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="sku" id="sku" class="form-input"
                        value="{{ old('sku', $material->sku) }}" placeholder="e.g. CMT-001" />
                ...
                <div class="form-group">
                    ...
                    <select name="unit" id="unit" class="form-select" required>
                        ...
                        <option value="m3" {{ old('unit', $material->unit) == 'm3' ? 'selected' : '' }}>Cubic Meter (m³)</option>
                        <option value="sqm" {{ old('unit', $material->unit) == 'sqm' ? 'selected' : '' }}>Square Meter (m²)</option>
                        <option value="m" {{ old('unit', $material->unit) == 'm' ? 'selected' : '' }}>Meter (m)</option>
                        <option value="ft" {{ old('unit', $material->unit) == 'ft' ? 'selected' : '' }}>Feet (ft)</option>
            ...
            <div class="form-group mt-5">
                ...
                <textarea name="description" id="description" class="form-textarea" rows="3">{{ old('description', $material->description) }}</textarea>
            </div>

            <div class="mt-8 flex items-center gap-4">
                <button type="submit" class="btn btn-primary px-10">Update Material</button>
                <button type="button" onclick="window.location.href='{{ route('admin.procurement.materials.index') }}'"
