...
<head>
    <meta charset="UTF-8">
    <title>Material Transfer - {{ $materialTransfer->transfer_number }}</title>
    <style>
        @page { margin: 0; }
    ...
    <div class="wrapper">
        ...
        <div class="content">
            <div style="margin-bottom:15px;">
                <div class="doc-number">{{ $materialTransfer->transfer_number }}</div>
            </div>

            <table class="meta-table">
                <tr>
                    <th>Transfer Type</th>
                    <td>{{ $materialTransfer->transfer_type_label }}</td>
                    <th>Status</th>
                    <td style="text-transform:capitalize;">{{ $materialTransfer->status }}</td>
                </tr>
                <tr>
                    <th>From</th>
                    <td>{{ $materialTransfer->from_location_label }}</td>
                    <th>To</th>
                    <td>{{ $materialTransfer->to_location_label }}</td>
                </tr>
                <tr>
                    <th>Transfer Date</th>
                    <td colspan="3">{{ $materialTransfer->transfer_date->format('d M Y') }}</td>
                </tr>
            </table>
            ...
            <table class="items-table">
                ...
                <tbody>
                    @forelse($materialTransfer->items as $i => $item)
                        <tr>
                            <td>{{ $i + 1 }}</td>
